...
class FieldMonitoringView(GenericAPIView):
    def post(self,request,field_id):
        try:
            ...
            return Response ({'message' : f'the field with id {field_id} does not exist'},status=404)
        #get IOT json data (currently get thingspeak api data)
        headers = {'method':'GET',"Accept":'application/json'}
        response = requests.get(THINGSPEAK_BASE_API_URL+'feeds/last.json',headers=headers)
        #get field data
