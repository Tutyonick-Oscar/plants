...
class WeatherService:
    ...
    def analyze_growing_conditions(self, latitude: float, longitude: float, crop_type: str) -> Dict:
    ...
    def _get_crop_recommendations(self, crop_type: str, conditions: Dict) -> List[str]:
        ...
        if not conditions["temperature_suitable"]:
            recommendations.append("Température non optimale - surveillez de près la croissance")
        ...
        if conditions["rain_expected"]:
            recommendations.append("Pluie prévue - planifiez les travaux en conséquence")
